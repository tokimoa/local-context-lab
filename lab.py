...
def parse_todos(text: str) -> list[dict]:
    """出力からtodos配列を取り出す（フェンス・前後説明に耐性）。失敗時は空リスト。"""
    ...
    return [t for t in todos if isinstance(t, dict) and t.get("item")]
...
def run_full(runner: Runner, series: list[dict], tally: Tally) -> list[dict]:
    ...
    prompt = (f"以下の{len(series)}件の会議書き起こしを読み、全会議のTODO（宿題・アクション"
              ...
              f"\n# 出力形式（このJSONのみ）\n{TODO_JSON_SPEC}")
    return parse_todos(runner.generate(prompt, tally, "full", max_tokens=8192))
...
def run_note(runner: Runner, series: list[dict], tally: Tally) -> list[dict]:
    # 構造化ノート: 会議ごとの抽出結果をハーネスが蓄積し、ノート自体が成果物になる
    return [t for todos in _extract_each(runner, series, tally, "note") for t in todos]


def run_subagent(runner: Runner, series: list[dict], tally: Tally) -> list[dict]:
    ...
    with out_path.open("a") as f:
        for si, series in enumerate(series_list):
            if si in done:
                continue
            tally = Tally()
            candidates = APPROACHES[args.approach](runner, series, tally)
            row = {
